...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    @Override
    public void onClick(View view) {//버튼을 누르면 호출되는 메소드, view에 눌러진 버튼의 참조값이 들어있는데,
        /*
        //눌러진 버튼의 참조값이 어떤 버튼인지에 따라 분기하는 코드
        if(view == toCanada){
        ...
        */
        //눌러진 버튼의 아이디 값(자동 부여된 정수값)을 읽어와서 R 클래스에 등록된 아이디값과 비교해서 분기하기
        switch(view.getId()){
            case R.id.toCanada:
                //CanadaActivity를 활성화 시킬 수 있는 의도를 가진 객체 생성
                Intent intent = new Intent(this, CanadaActivity.class);
